...
export default function SeguridadSalud(props) {
  const classes = useStyles();
  const { ...rest } = props;
  const { id: idItem  } = useParams();

  /*-------------------------Calidad de Vida------------------------------------------- */
  const [dataSeguridadSalud, setDataSeguridadSalud] = useState([]);
  const [titulo, setTitulo] = useState("")
  const [open, setOpen] = useState(false)

  useEffect(() => {
    const llamadoSeguridadSalud = async () => {

      setOpen(true);
      const {data} = await axios.get(`${CONFIG.endpoints.strapi}/seguridad-saluds/${idItem}`)
    setTitulo(data.title)

      const arr = await data
        ?.contenido_seguridad_saluds.map((item, i) => {
        ...
        setDataSeguridadSalud(arr);
        setOpen(false);
    };
    llamadoSeguridadSalud();
  }, []);
  ...
  return (
    <div>
      ...
      <Parallax
        small
        image={require("assets/img/imagen7.jpg").default}
      />
