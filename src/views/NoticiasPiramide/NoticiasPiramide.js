...
export default function CalidadPage(props) {
  ...
  /*-------------------------Calidad de Vida------------------------------------------- */
  const [dataNoticiasPiramide, setDataNoticiasPiramide] = useState([]);
  const [titulo, setTitulo] = useState("");
  const [open, setOpen] = useState(false);

  useEffect(() => {
    const llamadoNoticiasPiramide = async () => {
      setOpen(true);
      const { data } = await axios.get(
        `${CONFIG.endpoints.strapi}/noticias-sections/${idItem}`
      );
      setTitulo(data.title);
      const arr = await data?.contenido_noticias_sections
        .map((item, i) => {
      ...
      setDataNoticiasPiramide(arr);
      setOpen(false);
    };
    llamadoNoticiasPiramide();
  }, []);

  return (
    <div>
      <Backdrop style={{ zIndex: 99, color: "#fff" }} open={open}>
      ...
      <Header
        ...
        {...rest}
      />
      <Parallax small image={require("assets/img/noticias3.jpg").default} />
      <div className={classNames(classes.main, classes.mainRaised)}>
        ...
        <div>
          ...
          {dataNoticiasPiramide?.map((item, i) => (
            <Grid
              container
              key={i}
              style={{
